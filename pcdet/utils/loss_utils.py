...
class SigmoidFocalClassificationLoss(nn.Module):
    ...
    def __init__(self, gamma: float = 2.0, alpha: float = 0.25):
        ...
        self.alpha = alpha
        self.gamma = gamma
        # total_targets = {'road_vehicle': 1056, 'person': 11054, 'train': 902}

        # # 计算每个类别的权重
        # weights_dict = {cls: max(total_targets.values()) / total_targets[cls] for cls in total_targets}

        # # 构建权重张量
        # self.weights_tensor = torch.tensor([weights_dict[cls] for cls in total_targets.keys()])
...
# class SigmoidFocalClassificationLoss(torch.nn.Module):
#     def __init__(self, gamma: float = 2.0, alpha: float = 0.25):
#         super(SigmoidFocalClassificationLoss, self).__init__()
#         self.alpha = alpha
#         self.gamma = gamma
...
#         if weights.ndim == 2 or (weights.ndim == 1 and target.ndim == 2):
#             weights = weights.unsqueeze(-1)
...
# # 构建权重张量
# weights_tensor = torch.tensor([weights_dict[cls] for cls in total_targets.keys()])

# # 初始化损失函数
# loss_fn = SigmoidFocalClassificationLoss()

# # 假设 input 和 target 是模型输出的 logits 和目标的 one-hot 编码
# input = torch.randn(1, 3, 10)  # 假设批大小为 1，3 个 anchor，10 个类别
# target = torch.randint(0, 2, (1, 3, 10), dtype=torch.float32)  # 随机生成的目标 one-hot 编码

# # 计算损失
# loss = loss_fn(input, target, weights_tensor)
...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        ...
        # input = input.permute(0, 2, 1)
        # target = target.argmax(dim=-1)
        # print("Max:", input.max().item())
        # print("Max:", target.max().item())
        loss = F.cross_entropy(input, target, reduction='none') 
 
        total_loss = loss.sum()
        num_pixels = loss.numel()  # 总的像素数（包括batch size、通道数、高度和宽度）

        # 计算每个像素的平均损失
        average_loss_per_pixel = total_loss / num_pixels

        # # 打印总损失和平均每个像素的损失
        # print("Total loss:", total_loss.item())
        # print("Average loss per pixel:", average_loss_per_pixel.item())
        return average_loss_per_pixel
...
class FocalLoss_depth(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        ...
        target_simplified = torch.argmax(target, dim=1)
        # Compute loss
        loss = self.loss_func(input, target_simplified)
