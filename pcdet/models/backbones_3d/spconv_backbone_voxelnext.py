...
class VoxelResBackBone8xVoxelNeXt(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        print('lll',(x_conv4.indices).shape)##torch.Size([26021, 4]
        print('ppp',x_conv4.features.shape)##torch.Size([26021, 256]

        out = self.bev_out(x_conv4)
        # print(out.features.shape)##torch.Size([5634, 256])
        # print(out.indices.shape)##torch.Size([5634, 3])
        

        out = self.conv_out(out)
        out = self.shared_conv(out)
        # print(out.features.shape)##torch.Size([8856, 256])
        # print(out.indices.shape)##torch.Size([8856, 3])

        batch_dict.update({
            'encoded_spconv_tensor': out,
            'encoded_spconv_tensor_x_conv4': x_conv4,
        ...
        return batch_dict
class VoxelResBackBone8xVoxelNeXt_WUXIACY(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
    ...
    def forward(self, batch_dict):
        ...
        x_conv4 = self.conv4(x_conv3)
        out = x_conv4
        # print((x_conv4.features).shape)
        # x_conv5 = self.conv5(x_conv4)
        # # print((x_conv5.features).shape)

        # x_conv6 = self.conv6(x_conv5)
        # # print((x_conv6.features).shape)
        # # print(torch.cat([x_conv4.features, x_conv5.features, x_conv6.features]).shape)
        # x_conv5.indices[:, 1:] *= 2
        # x_conv6.indices[:, 1:] *= 4
        # x_conv4 = x_conv4.replace_feature(torch.cat([x_conv4.features, x_conv5.features, x_conv6.features]))
        ...
        # x_conv4.indices = torch.cat([x_conv4.indices, x_conv5.indices, x_conv6.indices])
        # print('lll',(x_conv4.indices).shape)
        # print('ppp',x_conv4)

        # out = self.bev_out(x_conv4)
        # print(out.features.shape)
        # print(out.indices.shape)
        ...
        return batch_dict
class VoxelResBackBone8xVoxelNeXt_WUXISHU(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
